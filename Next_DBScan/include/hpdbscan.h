...
class HPDBSCAN {
    ...
    Rules local_dbscan(Clusters& clusters, const SpatialIndex<T>& index) {
        ...
        printDiffTime("hpdbscan.h - local_dbscan", start_timestamp);
        return rules;
    }
    ...
    void merge_halos(Clusters& clusters, Rules& rules, const SpatialIndex<T>& index) {
        ...
        printDiffTime("hpdbscan.h (merge_halos) - MPI_Alltoall", start_timestamp_mpi);
        ...
        printDiffTime("hpdbscan.h (merge_halos) - MPI_Alltoallv", start_timestamp_mpi);
        ...
        printDiffTime("hpdbscan.h - merge_halos", start_timestamp);
    }
    ...
    void distribute_rules(Rules& rules) {
        ...
        auto start_timestamp_mpi = getTime();
        MPI_Alltoall(send_counts, 1, MPI_INT, recv_counts, 1, MPI_INT, MPI_COMM_WORLD);
        printDiffTime("hpdbscan.h (distribute_rules) - MPI_Alltoall", start_timestamp_mpi);
        ...
        printDiffTime("hpdbscan.h (distribute_rules) - MPI_Alltoallv", start_timestamp_mpi);
        for (size_t i = 0; i < total; i += 2) {
            rules.update(incoming_rules[i], incoming_rules[i + 1]);
        }

        printDiffTime("hpdbscan.h - distribute_rules", start_timestamp);
    }
    #endif
    ...
    void apply_rules(Clusters& clusters, const Rules& rules) {
        ...
        printDiffTime("hpdbscan.h (#pragma omp parallel for) - apply_rules", start_timestamp);
    }
    ...
    void summarize(const Dataset& dataset, const Clusters& clusters) const {
        ...
        printDiffTime("hpdbscan.h (summarize) - MPI_Gather", start_timestamp_mpi);
        ...
        printDiffTime("hpdbscan.h (summarize) - MPI_Gatherv", start_timestamp_mpi);
        ...
        printDiffTime("hpdbscan.h (summarize) - MPI_Reduce", start_timestamp_mpi);
        ...
        log("\tClusters:       " + std::to_string((metrics[1] ? unique_clusters.size() - 1 : unique_clusters.size())) + "\n");
        log("\tCluster points: " + std::to_string(metrics[0]) + "\n");
        log("\tNoise points:   " + std::to_string(metrics[1]) + "\n");
        log("\tCore points:    " + std::to_string(metrics[2]) + "\n");
        #ifdef WITH_MPI
        }
        #endif

        printDiffTime("hpdbscan.h - summarize", start_timestamp);
    }
    #endif
...
public:
    ...
    Clusters cluster(Dataset& dataset, int threads=omp_get_max_threads()) {
        ...
        Rules rules = local_dbscan(clusters, index);
            ...
            if (m_rank == 0) {
            #endif
            // std::cout << "\tDBSCAN.. [OK] in " << omp_get_wtime() - start << std::endl;
            log("\tDBSCAN.. [OK] in " + std::to_string(omp_get_wtime() - start) + "\n");
            #ifdef WITH_MPI
            }
            #endif
            start = omp_get_wtime();
            ...
            #ifdef WITH_OUTPUT
            if (m_rank == 0) {
                // std::cout << "\tMerging halos... [OK] in " << omp_get_wtime() - start << std::endl;
                log("\tMerging halos... [OK] in " + std::to_string(omp_get_wtime() - start) + "\n");
                start = omp_get_wtime();
            }
            #endif
        #endif
        ...
        apply_rules(clusters, rules);
            ...
            if (m_rank == 0) {
            #endif
            // std::cout << "\tAppyling rules... [OK] in " << omp_get_wtime() - start << std::endl;
            log("\tAppyling rules... [OK] in " + std::to_string(omp_get_wtime() - start) + "\n");
            #ifdef WITH_MPI
            }
            #endif
        #endif
        ...
        index.recover_initial_order(clusters);
            ...
            if (m_rank == 0) {
            #endif
            // std::cout << "\tRecovering order... [OK] in " << omp_get_wtime() - start << std::endl;
            log("\tRecovering order... [OK] in " + std::to_string(omp_get_wtime() - start) + "\n");
            #ifdef WITH_MPI
            }
            #endif
            start = omp_get_wtime();
        ...
        summarize(dataset, clusters);
            ...
            log("Total time: " + std::to_string(omp_get_wtime() - execution_start) + "\n");
            #ifdef WITH_MPI
            }
            #endif
        #endif
