...
private:
    ...
    void redistribute_dataset() {
        ...
        MPI_Ialltoall(send_counts, 1, MPI_INT, recv_counts, 1, MPI_INT, MPI_COMM_WORLD, &request1);
        ...
        auto start_timestamp_mpi_1 = getTime();
        MPI_Ialltoallv(
            static_cast<T*>(m_data.m_p), send_counts, send_displs, MPI_Types<T>::map(),
            point_buffer, recv_counts, recv_displs, MPI_Types<T>::map(), MPI_COMM_WORLD, &request2
        );
        auto start_timestamp_mpi_2 = getTime();
        MPI_Ialltoallv(
            m_initial_order.data(), send_counts_labels, send_displs_labels, MPI_UNSIGNED_LONG,
            order_buffer.data(), recv_counts_labels, recv_displs_labels, MPI_UNSIGNED_LONG, MPI_COMM_WORLD, &request3
        );
        MPI_Wait(&request2, MPI_STATUS_IGNORE);
        printDiffTime("spatial_index (redistribute_dataset) - 2° MPI_Alltoall", start_timestamp_mpi_1);
        MPI_Wait(&request3, MPI_STATUS_IGNORE);
        printDiffTime("spatial_index (redistribute_dataset) - 3° MPI_Alltoall", start_timestamp_mpi_2);
