...
private:
    ...
    void redistribute_dataset() {
        ...
        for (int i = 0; i < m_size; i++) {
            if (i != m_rank) {
                MPI_Isend(&send_counts[i], 1, MPI_INT, i, TAG, MPI_COMM_WORLD, &send_requests[i]);
                MPI_Irecv(&recv_counts[i], 1, MPI_INT, i, TAG, MPI_COMM_WORLD, &recv_requests[i]);
            } else {
        ...
        start_timestamp_mpi = getTime();
        MPI_Request send_requests_2[m_size];
        MPI_Request recv_requests_2[m_size];

        for (int i = 0; i < m_size; i++) {
            if (i != m_rank) {
                MPI_Isend(static_cast<T*>(m_data.m_p) + send_displs[i], send_counts[i], MPI_Types<T>::map(), i, TAG, MPI_COMM_WORLD, &send_requests[i]);
                MPI_Irecv(point_buffer + recv_displs[i], recv_counts[i], MPI_Types<T>::map(), i, TAG, MPI_COMM_WORLD, &recv_requests[i]);

                MPI_Isend(&m_initial_order[send_displs_labels[i]], send_counts_labels[i], MPI_UNSIGNED_LONG, i, TAG, MPI_COMM_WORLD, &send_requests_2[i]);
                MPI_Irecv(&order_buffer[recv_displs_labels[i]], recv_counts_labels[i], MPI_UNSIGNED_LONG, i, TAG, MPI_COMM_WORLD, &recv_requests_2[i]);
            } else {
                std::copy(static_cast<T*>(m_data.m_p) + send_displs[i], static_cast<T*>(m_data.m_p) + send_displs[i] + send_counts[i], point_buffer + recv_displs[i]);
                for (int j = 0; j < recv_counts_labels[i]; j++) {
                    order_buffer[recv_displs_labels[i] + j] = m_initial_order[send_displs_labels[i] + j];
                }
            }
        }
        ...
        for (int i = 0; i < m_size; i++) {
            if (i != m_rank) {
                MPI_Wait(&send_requests[i], MPI_STATUS_IGNORE);
                MPI_Wait(&recv_requests[i], MPI_STATUS_IGNORE);
                MPI_Wait(&send_requests_2[i], MPI_STATUS_IGNORE);
                MPI_Wait(&recv_requests_2[i], MPI_STATUS_IGNORE);
            }
        }
