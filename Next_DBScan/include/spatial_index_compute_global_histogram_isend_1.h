...
private:
    ...
    size_t* getOffset(size_t *basicAddress, int index, int *recvCounts)
    {
        int pos = 0;
	...
	auto start_timestamp_mpi = getTime();
        ...
        while (i != m_size){
            int index;
            MPI_Waitany(m_size, recv_requests_elements, &index, MPI_STATUS_IGNORE);
            i++;
            if(m_rank != index){
	...
	start_timestamp_mpi = getTime();
        for (int i = 0; i < m_size; i++){
            if (i != m_rank){
                ...
                MPI_Irecv(recv_buffer_address, recv_counts[i], MPI_UNSIGNED_LONG, i, TAG, MPI_COMM_WORLD, &recv_requests[i]);
                recv_buffer_address += recv_counts[i];
            }
        }
        ...
        #pragma omp parallel for
        for (int j = 0; j < send_counts; j += 2) {
            global_histogram[send_buffer[j]] += send_buffer[j + 1];
        }
        ...
        while (i != m_size){
            int index;
            MPI_Waitany(m_size, recv_requests, &index, MPI_STATUS_IGNORE);
            i++;
            if(index != m_rank){
                size_t * startingOffset = getOffset(recv_buffer.data(), index, recv_counts);
                #pragma omp parallel for
                for (int j = 0; j < recv_counts[index]; j += 2) {
                    global_histogram[startingOffset[j]] += startingOffset[j + 1];
                }
            }
