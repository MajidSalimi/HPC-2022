	...
	auto start_timestamp_mpi = getTime();
        for (int i = 0; i < m_size; i++){
            if (i != m_rank){
                ...
                MPI_Ibcast(&send_counts, 1, MPI_INT, m_rank, MPI_COMM_WORLD, &recv_requests_elements[i]);
            }
        }
	...
	start_timestamp_mpi = getTime();
        for (int i = 0; i < m_size; i++){
            if (i != m_rank){
                ...
                MPI_Ibcast(send_buffer.data(), m_cell_histogram.size() * 2, MPI_UNSIGNED_LONG, m_rank, MPI_COMM_WORLD, &requests[i]);
            }
        }
        ...
        {
            int index;
            MPI_Waitany(m_size, requests, &index, MPI_STATUS_IGNORE);
            i++;
            if (index != m_rank){
