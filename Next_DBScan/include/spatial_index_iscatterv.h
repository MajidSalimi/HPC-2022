...
private:
    ...
    void redistribute_dataset() {
        ...
        for (int i = 0; i < m_size; i++) {
            MPI_Iscatter(&send_counts, 1, MPI_INT, &recv_counts[i], 1, MPI_INT, i, MPI_COMM_WORLD, &requests[i]);
        }
        MPI_Waitall(m_size, requests, MPI_STATUSES_IGNORE);
        ...
        MPI_Request requests2[m_size];
        start_timestamp_mpi = getTime();
        for (int i = 0; i < m_size; i++) {
            MPI_Iscatterv(static_cast<T*>(m_data.m_p), send_counts, send_displs, MPI_Types<T>::map(),
            point_buffer + recv_displs[i], recv_counts[i], MPI_Types<T>::map(), i, MPI_COMM_WORLD, &requests[i]);

            MPI_Iscatterv(m_initial_order.data(), send_counts_labels, send_displs_labels, MPI_UNSIGNED_LONG,
            order_buffer.data() + recv_displs_labels[i], recv_counts_labels[i], MPI_UNSIGNED_LONG, i, MPI_COMM_WORLD, &requests2[i]);
        }
        MPI_Waitall(m_size, requests, MPI_STATUSES_IGNORE);
        MPI_Waitall(m_size, requests2, MPI_STATUSES_IGNORE);
        printDiffTime("spatial_index (redistribute_dataset) - 2° & 3° MPI_Alltoall", start_timestamp_mpi);
