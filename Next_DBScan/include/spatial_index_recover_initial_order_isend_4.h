...
public:
	...
	auto start_timestamp = getTime();
        ...
        for (int i = 0; i < m_size; i++){
                MPI_Isend(&send_counts[i], 1, MPI_INT, i, TAG, MPI_COMM_WORLD, &send_requests_elements[i]);
                MPI_Irecv(&recv_counts[i], 1, MPI_INT, i, TAG, MPI_COMM_WORLD, &recv_requests_elements[i]);
        }
        ...
        while (j != m_size){
            int i;
            MPI_Waitany(m_size, recv_requests_elements, &i, MPI_STATUS_IGNORE);
            j++;
            total_recv_items += recv_counts[i];
	...
	printDiffTime("spatial_index (recover_initial_order) - 1° Isend", start_timestamp_mpi);
        ...
        // actually transmit the data
        MPI_Request send_requests_data[m_size];
        MPI_Request recv_requests_data[m_size];
        MPI_Request recv_initial_order_requests[m_size];
        MPI_Request send_initial_order_requests[m_size];
        MPI_Request recv_cluster_requests[m_size];
        MPI_Request send_cluster_requests[m_size];

        size_t * order_buffer_address = order_buffer.data(); 
	...
	auto start_timestamp_mpi3 = getTime();
        ...
        for (int i = 0; i < m_size; i++){
            MPI_Isend(static_cast<T*>(m_data.m_p) + accumulator, send_counts_points[i], MPI_Types<T>::map(), i, TAG, MPI_COMM_WORLD, &send_requests_data[i]);
            MPI_Irecv(point_buffer + accumulator2, recv_counts_points[i], MPI_Types<T>::map(), i, TAG, MPI_COMM_WORLD, &recv_requests_data[i]);
            accumulator += send_counts_points[i];
            accumulator2 += recv_counts_points[i];

            MPI_Isend(initial_order_data, send_counts[i], MPI_Types<size_t>::map(), i, TAG, MPI_COMM_WORLD, &send_initial_order_requests[i]);
            MPI_Irecv(order_buffer_address, recv_counts[i], MPI_LONG, i, TAG, MPI_COMM_WORLD, &recv_initial_order_requests[i]);
            initial_order_data += send_counts[i];
            order_buffer_address += recv_counts[i];

            MPI_Isend(cluter_data, send_counts[i], MPI_Types<size_t>::map(), i, TAG, MPI_COMM_WORLD, &send_cluster_requests[i]);
            MPI_Irecv(cluster_buffer_data, recv_counts[i], MPI_LONG, i, TAG, MPI_COMM_WORLD, &recv_cluster_requests[i]);
            cluter_data += send_counts[i];
            cluster_buffer_data += recv_counts[i];
        }
          
        // assign the new data
        m_data.m_chunk[0] = total_recv_items;
        MPI_Waitall(m_size, recv_requests_data, MPI_STATUS_IGNORE);
	printDiffTime("spatial_index (recover_initial_order) - 2° Isend", start_timestamp_mpi);
        delete[] static_cast<T*>(m_data.m_p);
        m_data.m_p = point_buffer;
        point_buffer = nullptr;
        MPI_Waitall(m_size, recv_initial_order_requests, MPI_STATUS_IGNORE);
	printDiffTime("spatial_index (recover_initial_order) - 3° Isend", start_timestamp_mpi2);
        m_initial_order.swap(order_buffer);
        order_buffer.clear();
        MPI_Waitall(m_size, recv_cluster_requests, MPI_STATUS_IGNORE);
	printDiffTime("spatial_index (recover_initial_order) - 4° Isend", start_timestamp_mpi3);
        clusters.swap(cluster_buffer);
